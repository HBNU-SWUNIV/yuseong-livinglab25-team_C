...
class SmsService {
  ...
  async sendDailyWeatherMessage(recipients, weatherData, airQualityData) {
    try {
      // 메시지 생성
      const messageContent = this.template.generateDailyWeatherMessage(weatherData, airQualityData);
      ...
      // 메시지 상태 업데이트
      await this.updateMessageStatus(messageRecord.id, results);
  ...
  async sendEmergencyAlert(recipients, disasterData) {
    try {
      // 긴급 메시지 생성
      const messageContent = this.template.generateEmergencyMessage(disasterData);
  ...
  async sendCustomReminder(recipient, reminderData) {
    try {
      ...
      console.log(`맞춤 알림 발송: ${recipient.name} (${recipient.phone_number})`);
      ...
      // 메시지 상태 업데이트
      await Message.update(messageRecord.id, {
        status: result.success ? 'sent' : 'failed',
        sent_at: new Date(),
  ...
  async sendWelfareNotification(recipients, notificationData) {
    try {
      // 보건/복지 메시지 생성
      const messageContent = this.template.generateWelfareMessage(notificationData);
   ...
   * @returns {Promise<Object>} 발송 결과
   */
  async sendToMultipleRecipients(recipients, messageContent, messageId, messageType) {
    const results = {
      successCount: 0,
      failureCount: 0,
      details: []
   ...
   * @returns {Promise<Object>} 발송 결과
   */
  async sendToMultipleRecipientsWithRetry(recipients, messageContent, messageId, messageType) {
    const results = {
      successCount: 0,
      failureCount: 0,
      details: []
   ...
   * 메시지 상태 업데이트
   * @param {number} messageId 메시지 ID
   * @param {Object} results 발송 결과
   */
  async updateMessageStatus(messageId, results) {
    try {
      const status = results.failureCount === 0 ? 'sent' : 
                    results.successCount === 0 ? 'failed' : 'sent';
  ...
  generateMessagePreview(type, data) {
    ...
    switch (type) {
      case 'daily':
        message = this.template.generateDailyWeatherMessage(data.weather, data.airQuality);
        break;
      case 'emergency':
